...
static int  bma056_local_init(void);
static int  bma056_remove(void);

static int bma056_init_flag =0; // 0<==>OK -1 <==> fail
...
static struct sensor_init_info bma056_init_info = {
		.name = "bma056",
		.init = bma056_local_init,
		.uninit = bma056_remove,
...
/*----------------------------------------------------------------------------*/
int bma056_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int bma255_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err = bma255_create_attr(&(bma056_init_info.platform_diver_addr->driver)))
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = bma056_operate;
...
static int bma255_i2c_remove(struct i2c_client *client)
{
	...
	if(err = bma255_delete_attr(&(bma056_init_info.platform_diver_addr->driver)))
	{
...
static int bma056_remove(void)
{
    struct acc_hw *hw = bma056_get_cust_acc_hw();
...
int bmm050_wakeup(void)
{
	...
	while (try_times) {
		...
		dummy = 0;
		if((err = hwmsen_read_block(client, 0x4B, &dummy, 1)))
		{
			GSE_ERR("read block error: %d\n", err);
			//try_times--;
			//continue;
		}
		if (data[1] == dummy)
	...
	GSE_ERR("wake up result: %s, tried times: %d",
...
static int __init bma255_init(void)
{
	...
	hwmsen_gsensor_add(&bma056_init_info);
