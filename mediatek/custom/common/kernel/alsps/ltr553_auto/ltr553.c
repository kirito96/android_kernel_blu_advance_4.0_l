...
int ltr559_init_device(struct i2c_client *client)
{
    ...
    hwmsen_write_byte(client, 0x83, 0x0c); //from 0x0c to 0x0f   0x06              ///5 pulse
    ...
    {
        hwmsen_write_byte(client, 0x84, 0x00);          //ps measurement time 10 ms
...

{
    ...
    if(1 == ps_cali.valid)
    {
     ...
     APS_LOG(" ltr553_ps_set_thres ps_cali.valid =1:   APS_RW_PS_THRES_LOW_0= %d\n",databuf[1]);
       if(res <= 0)
        {
            goto EXIT_ERR;
        ...
        res = i2c_master_send(client, databuf, 0x2);
      APS_LOG(" ltr553_ps_set_thres ps_cali.valid =1:   APS_RW_PS_THRES_LOW_1= %d\n",databuf[1]);
       if(res <= 0)
        {
            goto EXIT_ERR;
            return ltr553_ERR_I2C;
        ...
        res = i2c_master_send(client, databuf, 0x2);
      APS_LOG(" ltr553_ps_set_thres ps_cali.valid =1:   APS_RW_PS_THRES_UP_0= %d\n",databuf[1]);
        if(res <= 0)
        {
            goto EXIT_ERR;
            return ltr553_ERR_I2C;
        ...
        res = i2c_master_send(client, databuf, 0x2);
      APS_LOG(" ltr553_ps_set_thres ps_cali.valid =1:   APS_RW_PS_THRES_UP_1= %d\n",databuf[1]);
        if(res <= 0)
        {
            goto EXIT_ERR;
            return ltr553_ERR_I2C;
...
static long ltr559_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case ALSPS_IOCTL_PS_CALI_RAW_DATA:
                APS_LOG("case ALSPS_IOCTL_PS_CALI_RAW_DATA: \n");
        if(err = ltr559_read_data_ps(obj->client, &obj->ps))
            {
                goto err_out;
            }
                APS_LOG("case ALSPS_IOCTL_PS_CALI_RAW_DATA: read finish\n");
            ...
            if(copy_to_user(ptr, &dat, sizeof(dat)))
            {
                ...
                goto err_out;
                  ...
                  APS_LOG("case ALSPS_IOCTL_PS_CALI_RAW_DATA: end\n");
          break;
#endif
        default:
...
static int ltr559_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(err = ltr559_create_attr(&(ltr553_init_info.platform_diver_addr->driver)))
    {
...
static int ltr559_i2c_remove(struct i2c_client *client)
{
    ...
    if(err = ltr559_delete_attr(&(ltr553_init_info.platform_diver_addr->driver)))
    {
