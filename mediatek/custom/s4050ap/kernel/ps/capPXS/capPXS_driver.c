...
extern s32 ektf2k_ps_operate(void *self, u32 command, void *buff_in, s32 size_in,
					   void *buff_out, s32 size_out, s32 *actualout);
extern s32 tpd_ps_operate(void *self, u32 command, void *buff_in, s32 size_in,
					   void *buff_out, s32 size_out, s32 *actualout);
...
struct capPXS_callback capPS_callback;
...
static int capPXS_probe(struct platform_device *pdev)
{
	...
	#if 1
	APS_LOG("capPXS_probe>>>%d\n",g_current_tp_ic);
	
	if (1==g_current_tp_ic)
	{
		obj_ps.sensor_operate = ektf2k_ps_operate;
	}else if (2==g_current_tp_ic){
	...
	if (1==g_current_tp_ic){
		capPS_callback.capPXS_operate=ektf2k_ps_operate;
	}else if (2==g_current_tp_ic){
		capPS_callback.capPXS_operate = tpd_ps_operate;
	}
	capPS_callback.capPXS_attr_show= capPXS_show_ps;
	capPS_callback.capPXS_attr_store = NULL;
	capPS_callback.capPXS_ioctl = capPXS_unlocked_ioctl;
	capPS_callback.capPXS_eint_registration = capPXS_eint_work;

	capPXS_CB_registration(&capPS_callback);
