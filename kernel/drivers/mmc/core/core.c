				  ...
				  struct mmc_request *mrq)
{
    ...
    if ((mrq->cmd->arg == 0) && (mrq->data) && 
        ...
        cmd = mrq->cmd;
        if (!cmd->retries || mmc_card_removed(host->card))
            break;
            ...
            || cmd->opcode == MMC_WRITE_MULTIPLE_BLOCK)
        {
            ...
            stop = mrq->stop;
            if (stop) {
                pr_debug("XXX STOP<%d><0x%x> Error<%d> Resp0<0x%x> Resp1<0x%x> Resp2<0x%x> Resp3<0x%x>", 
                    stop->opcode, stop->arg, stop->error, stop->resp[0], stop->resp[1], stop->resp[2], stop->resp[3]);

                if (stop->resp[0] & R1_ERROR) {
                    goto retry;
                }
            }
